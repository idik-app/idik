...
export function DatetimeLocalPicker({
  ...
  /** Hanya dipakai jika `appearance="drawer"`. */
  isLight?: boolean;
}) {
  ...
  const dayPickerClass = cn(
    "rdp-root text-[12px]",
    isDrawer && isLight ? "text-slate-900" : "text-white",
  );
  ...
  const panelClass = cn(
    ...
    isDrawer && isLight
      ...
      : isDrawer
        ...
        : "border-white/15 bg-[#0a1628]/98 backdrop-blur-sm",
  );
  ...
  const iconClass = cn(
    ...
    isDrawer && isLight ? "text-cyan-600" : isDrawer ? "text-cyan-400/90" : "text-[#E8C547]/90",
  );
