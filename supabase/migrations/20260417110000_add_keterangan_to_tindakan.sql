-- Migration: Add keterangan field to tindakan table
-- Created at: 2026-04-17 11:00:00
...
BEGIN
    ...
    IF v_type = 'r' THEN
        -- Jika TABEL, tambah kolom langsung ke tabel tersebut
        ALTER TABLE public.tindakan_medik ADD COLUMN IF NOT EXISTS keterangan TEXT;
    ELSIF v_type = 'v' THEN
        ...
        ELSE
            -- Jika tidak ada dokter_id, coba gunakan dokter as dokter_nama jika ada
            IF EXISTS (
                SELECT 1 FROM information_schema.columns
                WHERE table_schema = 'public' AND table_name = 'tindakan' AND column_name = 'dokter'
            ) THEN
                 EXECUTE 'CREATE VIEW public.tindakan_medik AS SELECT *, dokter as dokter_nama FROM public.tindakan';
            ELSE
...
COMMENT ON COLUMN public.tindakan.keterangan IS 'Keterangan tambahan untuk tindakan (misal: pribadi, urgent, dll).';
