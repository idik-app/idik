...
export default function PasienImport() {
  ...
  const handleSubmit = async () => {
    if (!file) {
      show({
        type: "warning",
        message: "Pilih file Excel atau CSV terlebih dahulu.",
      });
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      if (data.ok && (data.imported ?? 0) > 0) {
        show({
          ...
          message:
            data.message ??
            `${data.imported} pasien berhasil diimpor ke Supabase.`,
        });
        await refresh();
        setFile(null);
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <p className="text-sm text-cyan-300/90">
                Upload file Excel (.xlsx, .xls) atau CSV. Data akan divalidasi
                dan disimpan ke Supabase (tabel pasien).
              </p>
              <p className="text-xs text-cyan-500">
                Kolom yang didukung: No. RM, Nama, JK, Tanggal Lahir, Alamat,
                No. HP, Pembiayaan, Kelas, Asuransi.
              </p>
                        ...
                        {result.errors && result.errors.length > 0 && (
                          <ul className="mt-2 text-xs text-cyan-300 max-h-24 overflow-y-auto space-y-1">
                            ...
                            {result.errors.length > 10 && (
                              <li>
                                ... dan {result.errors.length - 10} error
                                lainnya.
                              </li>
                            )}
                          </ul>
