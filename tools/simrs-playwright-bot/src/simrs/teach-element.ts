...
export async function buildStableSelector(
  ...
  const data = await el.evaluate((node) => {
    ...
    if (e instanceof HTMLInputElement || e instanceof HTMLTextAreaElement) {
      ...
      value = e.value || e.options[e.selectedIndex]?.text || "";
    } else {
      value = (e.textContent || "").trim().slice(0, 120);
    }
    return { tag, id, name, aria, placeholder, type, label, value };
  });
...
export async function waitForTeachClick(
  ...
  await page.evaluate(() => {
    (window as unknown as { __idikTeach?: { x: number; y: number } }).__idikTeach =
      undefined;
    ...
    await frame
      .evaluate(() => {
        ...
        const handler = (ev: MouseEvent) => {
          ...
          ev.stopPropagation();
          (
            window as unknown as { __idikTeach: { x: number; y: number } }
          ).__idikTeach = { x: ev.clientX, y: ev.clientY };
          document.removeEventListener("click", handler, true);
        };
        document.addEventListener("click", handler, true);
      })
  ...
  while (Date.now() < deadline) {
    ...
    for (const ctx of contexts) {
      ...
      // Prefer elementFromPoint in that context
