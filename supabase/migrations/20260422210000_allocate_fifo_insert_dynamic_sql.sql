-- Masih 22P02 uuid "53" walau wrapper kirim NULL: kemungkinan PL/pgSQL variable shadowing
-- (nama kolom keluaran RETURNS TABLE vs kolom tabel pemakaian di versi lama).
-- INSERT lewat EXECUTE ... USING menghindari substitusi variabel di statement statis.
...
begin
  ...
  loop
    ...
    execute
      ...
      values ($1::uuid, $2::numeric, $3::date, $4::text, $5::text, $6::text)
      returning id
      $ins$
...
comment on function public.allocate_pemakaian_fifo(uuid, numeric, text, text, text, date, text) is
  'FIFO; INSERT pemakaian via EXECUTE (hindari shadowing PL/pgSQL); tanpa kolom tindakan_id.';
