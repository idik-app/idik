  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json(
        { ok: false, message: "Master tindakan tidak ditemukan" },
        { status: 404 },
      );
    }
    ...
    const msg =
      err instanceof Error ? err.message : "Gagal memperbarui master tindakan";
    console.error("[PATCH /api/master-tindakan/:id]", err);
...
export async function DELETE(
  ...
  try {
    ...
    const msg =
      err instanceof Error ? err.message : "Gagal menghapus master tindakan";
    console.error("[DELETE /api/master-tindakan/:id]", err);
    return NextResponse.json({ ok: false, message: msg }, { status: 500 });
  }
}
