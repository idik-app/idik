...
export default function RuanganPagination() {
  const {
    currentPage,
    setCurrentPage,
    totalPages,
  ...
  return (
    <div
      ...
      role="navigation"
      aria-label="Paginasi daftar ruangan"
    >
      <p className="text-xs text-cyan-500/90 order-2 sm:order-1">
        ...
        dari <span className="text-yellow-400 font-medium">{total}</span>{" "}
        ruangan
      </p>
      ...
      <div className="flex items-center gap-2 text-xs text-cyan-400 order-3">
        <label htmlFor="ruangan-rows-per-page" className="whitespace-nowrap">
          Per halaman
        </label>
        <select
          id="ruangan-rows-per-page"
          value={rowsPerPage}
