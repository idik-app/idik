-- Hindari tabrakan nama: RETURNS TABLE membuat variabel PL/pgSQL dengan nama sama.
-- Kolom tabel public.pemakaian (inventaris_id, jumlah, tanggal, keterangan, tindakan_id, …)
-- tidak boleh punya nama identik dengan kolom keluaran fungsi — gunakan prefix out_.
...
comment on function public.allocate_pemakaian_fifo(uuid, numeric, text, text, text, date, text) is
  'FIFO; kolom keluaran ber-prefix out_* (tidak bentrok dengan kolom public.pemakaian).';
