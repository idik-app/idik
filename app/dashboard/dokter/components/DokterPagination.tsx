...
export default function DokterPagination() {
  const {
    currentPage,
    setCurrentPage,
    totalPages,
  ...
  return (
    <div
      ...
      role="navigation"
      aria-label="Paginasi daftar dokter"
    >
      <p className="text-xs text-cyan-500/90 order-2 sm:order-1">
        ...
        dari <span className="text-yellow-400 font-medium">{total}</span> dokter
      </p>
      ...
      <div className="flex items-center gap-2 text-xs text-cyan-400 order-3">
        <label htmlFor="dokter-rows-per-page" className="whitespace-nowrap">
          Per halaman
        </label>
        <select
          id="dokter-rows-per-page"
          value={rowsPerPage}
