...
function DialogSurface({ children }: { children: React.ReactNode }) {
  return (
    <motion.div
      ...
      className="relative w-[min(100%,22rem)] rounded-2xl border border-[#E8C547]/35 bg-[#050b14]/95 px-5 py-5 shadow-[0_0_40px_rgba(0,0,0,0.55),0_0_24px_rgba(232,197,71,0.12)] backdrop-blur-xl pointer-events-auto"
    >
      {children}
    </motion.div>
  );
    ...
    <AppDialogContext.Provider value={value}>
      ...
      {mounted && typeof document !== "undefined" && createPortal(
        <AnimatePresence>
          {open ? (
            <div
              className={cn("fixed inset-0 flex items-center justify-center p-4 pointer-events-auto", "z-[200000]")}
              role="presentation"
            >
              <motion.button
                type="button"
                aria-label="Tutup latar"
                className="absolute inset-0 bg-black/35 backdrop-blur-sm pointer-events-auto"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                onClick={() => {
