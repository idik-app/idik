...
import { X, CheckCircle2, Loader2, BarChart2, ChevronDown, LayoutGrid, List } from "lucide-react";
...
const INITIAL_DATA: SeveritySection[] = [
  {
    ...
    items: [
      { tindakan: "PTCA", level1: 20826800, level2: 35384800, level3: 43766800 },
      ...
      { tindakan: "IABP", level1: 21337800, level2: 30909500, level3: 36880800 },
      { tindakan: "ANGIOPLASTY PERIFER", level1: 15701600, level2: 19617500, level3: 25778600 },
      { tindakan: "OCLUDER", level1: "", level2: "", level3: 42675600 },
      { tindakan: "EVLA", level1: 13105300, level2: 16409100, level3: 34776200 },
  ...
  },
  {
    ...
    items: [
      { tindakan: "PTCA", level1: 22909500, level2: 40320000, level3: 50437500 },
      ...
      { tindakan: "IABP", level1: 25605400, level2: 37091400, level3: 44256900 },
      { tindakan: "ANGIOPLASTY PERIFER", level1: 18292300, level2: 22854400, level3: 30032100 },
      { tindakan: "OCLUDER", level1: "", level2: "", level3: 42675600 },
      { tindakan: "EVLA", level1: 15727900, level2: 19960900, level3: 40948500 },
  ...
  },
  {
    kelas: "TARIF KELAS I",
    items: [
      { tindakan: "PTCA", level1: 24992200, level2: 45373400, level3: 57108100 },
      ...
      { tindakan: "IABP", level1: 29972900, level2: 43273400, level3: 51633100 },
      { tindakan: "ANGIOPLASTY PERIFER", level1: 20883100, level2: 26091300, level3: 34285500 },
      { tindakan: "OCLUDER", level1: "", level2: "", level3: 46942100 },
      { tindakan: "EVLA", level1: 18347500, level2: 22972700, level3: 47775700 },
  ...
  },
  {
    kelas: "TARIF VIP",
    items: [
      { tindakan: "PTCA", level1: 28500000, level2: 52000000, level3: 65000000 },
      ...
      { tindakan: "IABP", level1: 35000000, level2: 48000000, level3: 60000000 },
      { tindakan: "ANGIOPLASTY PERIFER", level1: 24500000, level2: 30000000, level3: 40000000 },
      { tindakan: "OCLUDER", level1: "", level2: "", level3: 52000000 },
      { tindakan: "EVLA", level1: 21000000, level2: 26000000, level3: 54000000 },
        ...
        >
          ...
          <div className="flex items-center justify-between mb-4 shrink-0 px-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className={cn("text-xl font-black tracking-tight leading-none mb-1", isDark ? "text-amber-100" : "text-amber-900")}>
                  Severity Level Tarif
                </h3>
                <p className={cn("text-[10px] font-bold opacity-60 uppercase tracking-widest", isDark ? "text-amber-400" : "text-amber-700")}>
            ...
            <button
              ...
              className={cn(
                ...
                isDark ? "hover:bg-white/10 text-white/70 hover:text-white" : "hover:bg-black/5 text-slate-500 hover:text-slate-900"
              )}
            >
              <X size={20} />
            </button>
          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar scroll-smooth">
            <div className={cn(
              "grid gap-4",
              (!activeTab || activeTab === "ALL") ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"
            )}>
              {filteredSections.map((section, sIdx) => {
                const isCollapsed = collapsedSections[section.kelas];
                const realIdx = data.findIndex(d => d.kelas === section.kelas);
                        ...
                        >
                          <div className="flex flex-col p-2 gap-1 relative">
                            ...
                            <div className={cn(
                              ...
                              isDark ? "bg-slate-800/95 text-slate-400 backdrop-blur-sm" : "bg-slate-100/95 text-slate-500 backdrop-blur-sm"
                            )}>
                              <span>TINDAKAN</span>
                            ...
                            <div className="flex flex-col">
                              {section.items.map((item, iIdx) => {
                                const isAngioplasty = item.tindakan === "ANGIOPLASTY PERIFER";
                                return (
                                  <div
                                    ...
                                    className={cn(
                                      ...
                                      isAngioplasty 
                                        ? (isDark ? "bg-emerald-950/30 text-emerald-300" : "bg-emerald-50 text-emerald-700")
                                        : (iIdx % 2 === 0 
                                          ? (isDark ? "bg-white/[0.03]" : "bg-slate-100/50") 
                                          : "bg-transparent"),
                                      isDark ? "hover:bg-white/[0.05]" : "hover:bg-slate-200/50"
                                    )}
                                  >
                                    <span className={cn(
                                    ...
                                    {["level1", "level2", "level3"].map((field) => {
                                      const key = `${section.kelas}-${item.tindakan}-${field}`;
                                      const isEdited = lastEditedKey === key;
                                      return (
                                        <div key={field} className="relative">
                                          <span className="absolute left-1 top-1/2 -translate-y-1/2 text-[8px] font-bold opacity-30">Rp</span>
                                          <input
                                            type="text"
                                            value={item[field as keyof SeverityTarifItem]?.toLocaleString("id-ID") || ""}
                                            onChange={(e) => handleUpdateField(realIdx, iIdx, field as keyof SeverityTarifItem, e.target.value)}
                                            className={cn(
                                              ...
                                              isEdited && "bg-emerald-500/20 ring-1 ring-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)] animate-pulse"
                                            )}
                                            placeholder="—"
                                          />
                                        </div>
              ...
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest opacity-60">
                {isSaving ? (
                  <>
                    ...
                    <span className={isDark ? "text-amber-400" : "text-amber-600"}>Sinkronisasi Latar...</span>
                  </>
                ) : lastSaved ? (
                  <>
