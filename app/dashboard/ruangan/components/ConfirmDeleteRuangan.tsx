...
export default function ConfirmDeleteRuangan({
  itemName,
  onConfirm,
  onCancel,
}: ConfirmDeleteRuanganProps) {
        ...
        <div className="p-6 pb-4 relative">
          <div className="flex justify-between items-start gap-3">
            <div className="flex items-start gap-3 min-w-0">
              <div
                ...
                aria-hidden
              >
                <DoorOpen className="h-6 w-6 text-cyan-300" strokeWidth={2} />
              </div>
              <div className="min-w-0 text-left">
                <h2
                  id="confirm-ruangan-title"
                  className="text-lg font-semibold text-white tracking-tight"
                >
                  Hapus ruangan?
                </h2>
                <p
                  id="confirm-ruangan-desc"
                  className="mt-2 text-sm text-gray-300 leading-relaxed"
                >
                  <span className="text-cyan-200/90">Ruangan</span>{" "}
                  <span className="font-semibold text-yellow-300/95 break-words">
                    {itemName.trim() || "(tanpa nama)"}
                  </span>{" "}
                  akan dihapus permanen dari master data. Entri ini tidak lagi
                  muncul di daftar setelah konfirmasi.
                </p>
              </div>
            </div>
            <button
          ...
          <div className="mt-4 flex gap-2 rounded-lg border border-cyan-500/15 bg-cyan-950/20 px-3 py-2.5 text-left">
            ...
            <p className="text-xs text-cyan-100/80 leading-snug">
              Tindakan ini tidak dapat dibatalkan. Pastikan Anda memilih ruangan
              yang benar sebelum melanjutkan.
            </p>
          </div>
