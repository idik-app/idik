-- Pastikan baris seed `__wireframe_seed_rekap_demo__` memiliki field untuk Section B–D.
-- Isi logika sama dengan `20260501120000_update_iccu_wireframe_seed_section_b_d.sql` (idempotent).
-- Dipakai bila instance sebelumnya hanya menjalankan migrasi seed pertama sehingga tabel rekapitulasi
-- menampilkan Section A + hari perawatan tetapi survey mutu / diagnosis tetap nol.
--
...
begin
  ...
  if v_ru is null then
    raise notice 'ensure_wireframe_b_d: lewati — tidak ada ruangan slug iccu';
    return;
  end if;
  ...
  if not exists (
    ...
    raise notice 'ensure_wireframe_b_d: lewati — tidak ada baris seed (jalankan migrasi seed terlebih dahulu)';
    return;
  end if;
  ...
  raise notice 'ensure_wireframe_b_d: selesai — Section B/D pada baris seed tahun %', v_y;
end;
$$;
