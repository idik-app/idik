-- 22P02 uuid "57" setelah 22220000: di PL/pgSQL, RETURNING id pada EXECUTE INSERT
-- bisa tertafsir sebagai r.id (baris inventaris) alih-alih kolom pemakaian.id,
-- sehingga nilai bigint/ teks "57" dipaksa ke uuid.
-- Perbaikan: alias INSERT (AS p) + RETURNING p.id; loop SELECT pakai i.id AS inv_id.
...
begin
  ...
  for r in
    select i.id as inv_id, i.stok as inv_stok
    from public.inventaris i
    where i.lokasi = p_lokasi
      and i.master_barang_id = p_master_barang_id
      and i.stok > 0
  ...
  loop
    ...
    v_take := least(v_remaining, coalesce(r.inv_stok, 0));
    if v_take <= 0 then
      continue;
    end if;
    ...
    execute
      $ins$
      insert into public.pemakaian as p (
        inventaris_id,
        jumlah,
        tanggal,
        keterangan,
      ...
      returning p.id
      $ins$
    using r.inv_id, v_take, p_tanggal, p_keterangan, p_order_id, v_nama_barang
    into v_pemakaian_id;

    update public.inventaris i
    set stok = i.stok - v_take,
        updated_at = now()
    where i.id = r.inv_id;
...
comment on function public.allocate_pemakaian_fifo(uuid, numeric, text, text, text, date, text) is
  'FIFO; EXECUTE INSERT AS p RETURNING p.id (hindari RETURNING id = r.id / inventaris).';
