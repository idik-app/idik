...
export async function runSimrsRecipe(
  ...
  let steps: JobStep[] =
    opts.mode === "teach_element"
      ? [
          ...
          {
            id: "save_selector",
            label: "Simpan selector ke mapping",
            status: "pending",
          },
        ]
      : opts.mode === "tulis"
  ...
  await ensureSimrsSession({ headless: false, slowMoMs: 80 });
  ...
  try {
    ...
    if (opts.noRm) {
      // Best-effort: fill first visible search / RM field
      const search = page
        .locator(
      ...
      if ((await search.count()) > 0) {
        ...
        console.warn("[recipe] kotak cari RM tidak ditemukan — lanjut baca selector");
      }
    }
